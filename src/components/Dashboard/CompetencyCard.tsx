...
export default function CompetencyCard({ competency, xpEarned }: CompetencyCardProps) {
  ...
  return (
    <Link
      ...
      className="block bg-white rounded-lg p-6 shadow-sm hover:shadow-lg transition-shadow cursor-pointer"
    >
      <div className="flex items-start justify-between mb-4">
        <div>
          <div className="text-4xl mb-2">{competency.icon}</div>
          <h3 className="text-lg font-bold text-gray-900">{competency.name}</h3>
          <p className="text-sm text-gray-600">{competency.short_name}</p>
        </div>
      </div>
